...
public class doctorAdd extends JFrame implements FocusListener,ActionListener,ItemListener 
{
	...
	public doctorAdd(String s)
	{
		...
		tdadd=new TextArea(doctor.dadd);
	...
	public void read()
	{
		try
		{
			doctor.did=Integer.parseInt(tdid.getText());
			doctor.dfnm=tdfnm.getText();
			doctor.dmnm=tdmnm.getText();
			doctor.dlnm=tdlnm.getText();
			doctor.gen=cbg.getSelectedCheckbox().getLabel();
			doctor.dadd=tdadd.getText();
			//this.ad=tadd.getText();
			//System.out.println(tadd.getText());
			doctor.cno=tcno.getText();
			//doctor.jdt=tjdt.getText();
			doctor.dqul=tdqul.getText();
			//System.out.println(tsym.getText());

			doctor.ldt=tldt.getText();
			doctor.sft=sft.getSelectedItem();
			doctor.sts=sts.getSelectedItem();
			doctor.path=str;
		}
		catch (Exception e)
		{
	...
	public void focusLost(FocusEvent e)
	{
		...
		if(t==tdid)
		{
			...
			if(res)
			{
				...
				try
				{
					no=doctor.rs.getInt(1);
					dfn=doctor.rs.getString(2);
					dmn=doctor.rs.getString(3);
					dln=doctor.rs.getString(4);
					gen=doctor.rs.getString(5);
					dadd=doctor.rs.getString(6);
					cno=doctor.rs.getString(7);
					jdt=doctor.rs.getString(8);
					dqul=doctor.rs.getString(9);
					ldt=doctor.rs.getString(10);
					sft=doctor.rs.getString(11);
					sts=doctor.rs.getString(12);
					dir=doctor.rs.getString(14);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(b==ba)
		{
			try
			{
				...
				JOptionPane.showMessageDialog(null,"Doctor ID: "+doctor.did+"  Record Added");
			}
			catch (Exception e4)
			{
			}
		...
		if(b==bm)
		{
			...
			System.out.println("ganesh "+str);
				doctor.mod(doctor.did,f,str);
				JOptionPane.showMessageDialog(null,"Doctor ID: "+doctor.did+"  Record Modified");
		...
		if(b==bd)
		{
			try
			{
				read();
				doctor.del();
				JOptionPane.showMessageDialog(null,"Doctor ID: "+doctor.did+"  Record Deleted");
			}
			catch (Exception e6)
			{
			}
		...
		if(b==bl)
		{
			stab obj=null;
				try
				{
					obj=new stab(doctor.collectRows1());
				}
		...
		if(b==bs)
		{
			stab obj=null;
				try
				{
					...
					obj=new stab(doctor.collectRows(JOptionPane.showInputDialog("Doctor Last Name:")));
				}
